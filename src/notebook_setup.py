...
try:
    ...
    print(f"Error: Could not import MySQLLabMagic from mysql_lab_magic. Ensure file exists and src is in sys.path. Details: {e}")
...
try:
    project_root = os.path.dirname(os.getcwd()) # Assumes notebook is in 'notebooks/' subdir
    ...
    print(f"Warning: Could not automatically adjust sys.path. Ensure 'src' is importable. Error: {e}")
...
try:
    ...
    print(f"Warning: Could not load .env file from '{dotenv_path}'. Ensure it exists. Error: {e}")
...
if not all([db_user, db_password, db_name]):
    ...
    try:
        ...
        with db_engine.connect() as connection:
            print(f"Successfully created SQLAlchemy engine ('db_engine') and connected to database.")
