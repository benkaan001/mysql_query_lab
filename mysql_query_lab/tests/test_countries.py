...
def test_output(output, df=None):
    ...
    assert output.columns.tolist() == ['name', 'population', 'area'], \
        "The query did not return the correct column names."

    # Test that the query returns the correct values
    assert output['name'].isin(['Afghanistan', 'Algeria']).tolist(), \
    ...
    with pytest.raises(KeyError):
        try:
            ...
            df.query("invalid_column >= 25_000_000")[['name', 'population', 'area']]
        except:
            raise KeyError
